module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!user) return message.channel.send({ embed: { title: `Improper usage!`, color: 'RED', description: `Usage: ${client.guildCommandPrefixes.get(message.guild.id)}ban (user) (reason)`}})
        const userId = user.id
        const reason = args.slice(1).join(' ')
        const username = user.username
        try {
           user.ban({ reason: reason }).then(use => {  return message.channel.send({ embed: { title: `I have banned ${use.user.username} from this server!`, color: 'GREEN'}})})
